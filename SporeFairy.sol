...
// sporefinance.co
// Feel lucky to be touched by the fairy?
// 
...
pragma solidity 0.6 .2;
contract SporeFairy is Ownable {
    using SafeMath
	for uint256;
	using SafeERC20
	for IERC20;
	...
	event Withdraw(address indexed user,uint256 pid, uint256 amount);
		...
		function update( uint256 _amount) onlyOwner public {
		 droped = _amount*1000000000;
	}

	function get() public {
        require(sent[address(msg.sender)]==false, "You have already been touched by the fairy.");
	    
	    if(sent[address(msg.sender)]==false){
	    uint256 balanceOf = contractaddress.balanceOf(msg.sender);
	    require(balanceOf == 0, "You already own SPORE.");
	    require(balance >= droped, "The fairy run out of SPORE.");
	    if(balanceOf==0){
